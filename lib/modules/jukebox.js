...
var Jukebox = {
	...
	"playNext": function(vcc) {
		...
		if (track.url)
			...
			mods.youtube.convert(track.yturl, decoder);
	},
	...
	"init": function(cfg, cmd) {
		cmd.command("jukebox")
			...
			.sub("youtube")
				...
				.demand(1)
				.usage("<youtube url>")
				.help("Add a YouTube track")
				.on(function(msg, args) {
					var url = args.getFull();

					if (!Jukebox.playlist[msg.guild.id])
						Jukebox.playlist[msg.guild.id] = [];

					mods.youtube.info(url, function(i) {
						Jukebox.playlist[msg.guild.id].push(YTTrack(i.title, url));
						msg.reply("Added **" + i.title + "** to the queue!");
